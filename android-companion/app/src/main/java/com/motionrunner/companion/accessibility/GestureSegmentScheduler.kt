...
class GestureSegmentScheduler(private val service: AccessibilityService) {
    ...
    fun submit(command: TouchCommand, callback: (Boolean, String?) -> Unit) {
        handler.post { queue.addLast(Queued(command, callback)); dispatchNext() }
    }
    ...
    private fun dispatchNext() {
        ...
        if (command.pointerId != 0) { queued.callback(false, "Only pointer 0 is supported"); dispatchNext(); return }
        if (command.commandType == "BEGIN" && active) { queued.callback(false, "Pointer already active"); dispatchNext(); return }
        if (command.commandType != "BEGIN" && !active) { queued.callback(false, "Pointer is not active"); dispatchNext(); return }
    ...
    private companion object { const val SEGMENT_DURATION_MS = 24L }
}
